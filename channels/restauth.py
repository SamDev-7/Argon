...
class RestAuthInput(InputChannel):
    ...
    def blueprint(
        ...
        async def validate(request: Request) -> HTTPResponse:
            if not request.args:
                return response.json({"error": "unauthorized"}, status=401)
            if not request.args.get("token", None) == os.getenv("RASA_TOKEN"):
                return response.json({"error": "unauthorized"}, status=401)
        ...
        async def receive(request: Request) -> Union[ResponseStream, HTTPResponse]:
            ...
            if should_use_stream:
                ...
                if len(collector.messages) == 0:
                    return response.json({"error": "empty response from rasa"}, status=500)
